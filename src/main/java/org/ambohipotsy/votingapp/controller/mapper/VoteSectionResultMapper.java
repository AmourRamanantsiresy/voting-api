...
public class VoteSectionResultMapper {
    ...
    public VoteSectionResult toRest(VoteSection voteSection) {
        ...
        return VoteSectionResult.builder().name(voteSection.getName()).id(voteSection.getId()).voteCandidateResults(voteCandidates.stream().map(voteCandidateResultMapper::toRest).toList()).build();
    }
}
