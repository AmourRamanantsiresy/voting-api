...
public class VoteCandidateService {
    ...
    public List<VoteCandidate> getAll(String voteSectionId, String name) {
        if (name.isBlank() || name.isEmpty()) {
            return voteCandidateRepository.findAllByVoteSectionId(voteSectionId);
        }
        return voteCandidateRepository.findAllByVoteSectionIdAndNameContainsIgnoreCase(voteSectionId, name);
    }

    @Transactional
    public List<VoteCandidate> saveAll(String voteSectionId, List<VoteCandidate> voteCandidates) {
        VoteSection voteSection = voteSectionRepository.findById(voteSectionId).orElseThrow(() -> new NotFoundException("Vote section with id=" + voteSectionId + " not found."));
